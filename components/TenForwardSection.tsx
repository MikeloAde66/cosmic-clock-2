...
function CommentThread({ postId, canPost, onRequireSignIn }: { postId: string; canPost: boolean; onRequireSignIn: () => void }) {
  ...
  return (
    <div className="pt-3 mt-3 space-y-2 border-t border-cyan-500/10">
      ...
      {comments?.map((c) => (
        <div key={c.id} className="p-2 rounded-lg bg-slate-950/40">
      ...
      <form onSubmit={submitComment} className="flex gap-2 pt-1">
        <input
          ...
          className="flex-1 min-w-0 px-2 py-1.5 text-xs bg-black/40 border border-slate-800 rounded text-slate-100 placeholder-slate-600 outline-none focus:border-cyan-400/50"
        />
        <button
          type="submit"
          disabled={posting || (canPost && !draft.trim())}
          className="flex items-center justify-center px-2.5 py-1.5 text-xs font-bold rounded border border-cyan-400/40 bg-cyan-500/10 text-cyan-300 hover:bg-cyan-500/20 disabled:opacity-40 transition-colors"
...
function PostCard({ post, canPost, onRequireSignIn }: { post: CommunityPost; canPost: boolean; onRequireSignIn: () => void }) {
  ...
  return (
    <div className="p-4 border rounded-2xl border-slate-800/80 bg-slate-900/40 backdrop-blur-md transition-all duration-300 hover:border-cyan-400/40 hover:shadow-[0_0_25px_rgba(34,211,238,0.15)]">
      <div className="flex items-center justify-between gap-2">
        <span className="px-2.5 py-0.5 text-[10px] font-mono uppercase tracking-wide rounded-full border border-cyan-400/40 bg-cyan-500/10 text-cyan-300">
          {categoryLabel(post.category as CommunityCategory)}
        </span>
        <span className="text-[11px] text-slate-500">{relativeTime(post.created_at)}</span>
      </div>
      ...
      <div className="flex items-center justify-between pt-3 mt-3 border-t border-cyan-500/10">
        <span className="text-[11px] text-slate-500">{authorLabel(post.author, post.author_id)}</span>
        <button
          onClick={() => setExpanded((v) => !v)}
          className="flex items-center gap-1 text-[11px] font-mono uppercase text-slate-500 hover:text-cyan-300 transition-colors"
        >
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <select
                ...
                className="px-2 py-1 text-xs bg-black/40 border border-slate-800 rounded text-slate-300 outline-none focus:border-cyan-400/50"
              >
            ...
            <input
              ...
              placeholder="Title..."
              maxLength={200}
              className="w-full px-3 py-2 text-sm bg-black/40 border border-slate-800 rounded text-slate-100 placeholder-slate-600 outline-none focus:border-cyan-400/50"
            />
            <textarea
              value={content}
              ...
              rows={3}
              maxLength={10000}
              className="w-full px-3 py-2 text-sm bg-black/40 border border-slate-800 rounded text-slate-100 placeholder-slate-600 outline-none focus:border-cyan-400/50 resize-none"
            />
            <div className="flex items-center justify-between">
          ...
          <div className="px-4 py-3 border rounded-2xl border-slate-800/80 bg-slate-900/40 backdrop-blur-md">
            ...
            <div className="flex flex-wrap gap-1.5">
              {COMMUNITY_CATEGORIES.map((c) => (
                <span
                  key={c.value}
                  className="px-2.5 py-0.5 text-[10px] font-mono uppercase tracking-wide rounded-full border border-cyan-400/40 bg-cyan-500/10 text-cyan-300"
                >
                  {c.label}
                </span>
              ))}
