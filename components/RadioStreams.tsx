  ...
  return (
    <div className="relative z-10 w-full h-full p-8 overflow-y-auto">
      ...
      <div className="max-w-5xl mx-auto space-y-4">
        ...
        {/* Program Manager — the toggle-gated 24-hour automated broadcast
            clock (see RadioPlayerContext). Off by default; manually
            switching a station while it's on doesn't disable it — the
            next scheduled tick will just tune back to the current block. */}
        <div className="flex items-center justify-between px-3 py-2 border rounded-lg border-slate-800 bg-slate-900/40">
          <div className="flex items-center gap-2">
            <button
              onClick={toggleProgramManager}
              role="switch"
              aria-checked={programManagerEnabled}
              className={`relative h-5 w-9 rounded-full transition-colors ${
                programManagerEnabled ? 'bg-white' : 'bg-slate-700'
              }`}
            >
              <span
                className={`absolute top-0.5 h-4 w-4 rounded-full bg-slate-950 transition-transform ${
                  programManagerEnabled ? 'translate-x-4' : 'translate-x-0.5'
                }`}
              />
            </button>
            <span className="text-[11px] font-mono uppercase tracking-wide text-slate-300">Program Manager</span>
          </div>
          <span className="text-[10px] font-mono uppercase tracking-widest text-slate-500">
            {programManagerEnabled ? `On Air — ${activeProgramLabel ?? '…'}` : 'Off'}
          </span>
        </div>
